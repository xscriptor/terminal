...
restore_config_file() {
  ...
  if grep -Eq '^include[[:space:]]*=.*(x\.ini|madrid\.ini|lahabana\.ini|miami\.ini|paris\.ini|tokio\.ini|oslo\.ini|helsinki\.ini|berlin\.ini|london\.ini|praha\.ini|bogota\.ini)[[:space:]]*$' "$FILE"; then
